...
1. **Фильтровать элементы Коллекции по значениям полей:**
   ...
   - **Фильтр по наличию (☑️ checkbox):** Показывать элементы, у которых поле "Истина/Ложь" находится в активном положении. Идеально для фильтра "В наличии".
...
<div id="collection-filter-generator" class="generator-container">
  ...
  <div class="settings-block">
    ...
    <h3 class="settings-title">Шаг 3: Настройка полей для фильтрации</h3>

<div class="important-note">
    Добавьте поля из вашей коллекции, по которым вы хотите производить фильтрацию. Укажите <strong>Имя поля</strong> (как в Taptop, напр. <code>Цена</code>, <code>Категория</code>) или его технический ID. Затем настройте параметры фильтра.
...
<p>После нажатия кнопки "+ Добавить поле" появляется карточка для настройки фильтрации по конкретному полю из вашей Коллекции Taptop. Вот что означает каждая настройка:</p>
        ...
        <div class="field-row">
          ...
          <div class="field-group">
            ...
            <select class="select-styled filter-ui-type">
              <option value="none" selected>Не фильтровать по этому полю</option>
